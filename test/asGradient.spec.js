...
describe('AsGradient', () => {
  ...
  it('should init correctly', () => {
    ...
    expect(gradient.get(1).color.toString()).to.be.equal('rgba(255, 255, 255, 0.7)'); // get the second stop
  ...
  it('should work with non-position gradient string', () => {
    ...
    expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, yellow 0%, blue 100%)'); // test position

    gradient.fromString('linear-gradient(to bottom, yellow, red, blue)');
    expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, yellow 0%, red 50%, blue 100%)'); // test three color position

    gradient.fromString('linear-gradient(to bottom, yellow, red, white, blue)');
    expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, yellow 0%, red 33%, white 66%, blue 100%)'); // test four color position
  });
  ...
  describe('setPosition()', () => {
    it('should set position for color stop', () => {
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(to top, #fff, #333 50%, #000)'); // insert 3 color stops

      gradient.get(2).setPosition(0.4);
      expect(gradient.toString()).to.be.equal('linear-gradient(to top, #fff, #000 40%, #333 50%)'); // insert 3 color stops
    });
  });
  ...
  describe('reorder()', () => {
    it('should reorder color stop', () => {
      ...
      gradient.reorder();
      expect(gradient.toString()).to.be.equal('linear-gradient(to top, #fff, #333 50%, #ddd 80%, #000)'); // insert 4 color stops
      gradient.append('#aaa', 0.1);

      expect(gradient.toString()).to.be.equal('linear-gradient(to top, #fff, #333 50%, #ddd 80%, #000 100%, #aaa 10%)'); // insert 5 color stops
      gradient.reorder();
      expect(gradient.toString()).to.be.equal('linear-gradient(to top, #fff, #aaa 10%, #333 50%, #ddd 80%, #000)'); // insert 5 color stops
    });
  });
  ...
  describe('removeById()', () => {
    it('should remove color stop by id', () => {
      gradient.fromString('-webkit-linear-gradient(top, #123456, #ffffff, #654321)');

      gradient.removeById(2);

      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, #123456, #654321)');
    });
  });
  ...
  describe('toString()', () => {
    it('should to string with standare format', () => {
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, #2f2727, #1a82f7)'); // test toString standare

      gradient.fromString('-moz-linear-gradient(top, rgba(248,80,50,1) 0%, rgba(246,41,12,1) 51%, rgba(240,47,23,1) 71%, rgba(231,56,39,1) 100%)');
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, rgb(248, 80, 50), rgb(246, 41, 12) 51%, rgb(240, 47, 23) 71%, rgb(231, 56, 39))'); // test toString 2
    });

    it('should prefix the string', () => {
      gradient.fromString('-webkit-linear-gradient(top, #2F2727, #1a82f7)');
      expect(gradient.toString('-webkit-')).to.be.equal('-webkit-linear-gradient(top, #2f2727, #1a82f7)'); // test toString webkit
      expect(gradient.toString('-moz-')).to.be.equal('-moz-linear-gradient(top, #2f2727, #1a82f7)'); // test toString moz
      expect(gradient.toString('-o-')).to.be.equal('-o-linear-gradient(top, #2f2727, #1a82f7)'); // test toString o
      expect(gradient.toString('-ms-')).to.be.equal('-ms-linear-gradient(top, #2f2727, #1a82f7)'); // test toString ms
      expect(gradient.toString('-undefined-')).to.be.equal('linear-gradient(to bottom, #2f2727, #1a82f7)'); // test toString undefined prefix
    });

    it('should to string with angle', () => {
      gradient.fromString('-webkit-linear-gradient(top, #2F2727, #1a82f7)');
      expect(gradient.toStringWithAngle('to right', '-webkit-')).to.be.equal('-webkit-linear-gradient(left, #2f2727, #1a82f7)'); // test toStringWithAngle
      expect(gradient.toStringWithAngle('to right')).to.be.equal('linear-gradient(to right, #2f2727, #1a82f7)'); // test toStringWithAngle standare
  ...
  describe('fromString()', () => {
    it('should parse from string', () => {
      ...
      expect(gradient.toString()).to.be.equal('-webkit-linear-gradient(50deg, #2f2727, #1a82f7)'); // test toString()

      gradient.fromString('-moz-linear-gradient(left, rgba(248,80,50,0.8) 1%, rgba(241,111,92,0.8) 50%, rgba(240,47,23,0.8) 71%, rgba(231,56,39,0.8) 99%)');
      expect(gradient._prefix).to.be.equal('-moz-'); // test moz prefix
      expect(gradient._type).to.be.equal('LINEAR'); // test linear type
      expect(gradient.angle()).to.be.equal(90); // test angle
      expect(gradient.get(0).color.toString()).to.be.equal('rgba(248, 80, 50, 0.8)'); // test first color stop
      expect(gradient.get(1).color.toString()).to.be.equal('rgba(241, 111, 92, 0.8)'); // test second color stop
      expect(gradient.get(2).color.toString()).to.be.equal('rgba(240, 47, 23, 0.8)'); // test third color stop
      expect(gradient.get(3).color.toString()).to.be.equal('rgba(231, 56, 39, 0.8)'); // test fourth color stop
      ...
      expect(gradient.toString()).to.be.equal('-moz-linear-gradient(left, rgba(248, 80, 50, 0.8) 1%, rgba(241, 111, 92, 0.8) 50%, rgba(240, 47, 23, 0.8) 71%, rgba(231, 56, 39, 0.8) 99%)'); // test toString()

      gradient.fromString('linear-gradient(to bottom, hsla(0,0%, 100%,0.8), hsla( 0,0%,96%,0.8) 47%, hsla(0,0%,93%,0.8))');
      expect(gradient._prefix).to.be.equal(null); // test standare prefix
      expect(gradient._type).to.be.equal('LINEAR'); // test linear type
      expect(gradient.angle()).to.be.equal(180); // test angle
      expect(gradient.get(0).color.toString()).to.be.equal('hsla(0, 0%, 100%, 0.8)'); // test first color stop
      expect(gradient.get(1).color.toString()).to.be.equal('hsla(0, 0%, 96%, 0.8)'); // test second color stop
      expect(gradient.get(2).color.toString()).to.be.equal('hsla(0, 0%, 93%, 0.8)'); // test third color stop
      expect(gradient.get(0).position).to.be.equal(null); // test first color stop position
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, hsla(0, 0%, 100%, 0.8), hsla(0, 0%, 96%, 0.8) 47%, hsla(0, 0%, 93%, 0.8))'); // test toString()
    });
    ...
    it('should get angle correctly from gradient string', () => {
      ...
      gradient.fromString('-webkit-linear-gradient(top right, #000000, #ffffff)');
      expect(gradient.angle()).to.be.equal(225); // test old top right

      gradient.fromString('-webkit-linear-gradient(right bottom, #000000, #ffffff)');
      expect(gradient.angle()).to.be.equal(315); // test old right bottom

      gradient.fromString('-webkit-linear-gradient(bottom left, #000000, #ffffff)');
  ...
  describe('option', () => {
    it('angleUseKeyword', () => {
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(135deg, yellow, blue)'); // test angleUseKeyword false 135deg

      gradient = new AsGradient('linear-gradient(0deg, yellow, blue)', {
        angleUseKeyword: false,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(0deg, yellow, blue)'); // test angleUseKeyword false 0deg

      gradient = new AsGradient('linear-gradient(0deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to top, yellow, blue)'); // test angleUseKeyword true 0deg

      gradient = new AsGradient('linear-gradient(45deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to top right, yellow, blue)'); // test angleUseKeyword true 45deg

      gradient = new AsGradient('linear-gradient(90deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to right, yellow, blue)'); // test angleUseKeyword true 90deg

      gradient = new AsGradient('linear-gradient(135deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to right bottom, yellow, blue)'); // test angleUseKeyword true 135deg

      gradient = new AsGradient('linear-gradient(180deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, yellow, blue)'); // test angleUseKeyword true 180deg

      gradient = new AsGradient('linear-gradient(225deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom left, yellow, blue)'); // test angleUseKeyword true 225deg

      gradient = new AsGradient('linear-gradient(270deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to left, yellow, blue)'); // test angleUseKeyword true 270deg

      gradient = new AsGradient('linear-gradient(315deg, yellow, blue)', {
        angleUseKeyword: true,
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to left top, yellow, blue)'); // test angleUseKeyword true 315deg
    });
    ...
    it('forceStandard', () => {
      ...
      expect(gradient.toString()).to.be.equal('-webkit-linear-gradient(left, #2f2727, #1a82f7)'); // test forceStandard false

      gradient = new AsGradient('-webkit-linear-gradient(right, #2F2727, #1a82f7)', {
        forceStandard: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to left, #2f2727, #1a82f7)'); // test forceStandard true
    });
    ...
    it('cleanPosition', () => {
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, yellow 0%, blue 100%)'); // test cleanPosition false

      gradient = new AsGradient('linear-gradient(to bottom, yellow 0%, blue 100%)', {
        cleanPosition: true
      });
      expect(gradient.toString()).to.be.equal('linear-gradient(to bottom, yellow, blue)'); // test cleanPosition true
    });
    ...
    it('forceColorFormat', () => {
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(to right, rgb(255, 255, 255) 0%, rgb(246, 246, 246) 47%, rgb(237, 237, 237) 100%)'); // test forceColorFormat false
      ...
      expect(gradient.toString()).to.be.equal('linear-gradient(to right, #fff 0%, #f6f6f6 47%, #ededed 100%)'); // test forceColorFormat true
    });
